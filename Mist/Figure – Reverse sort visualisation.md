...
> [!caption]
> A visualization of the 12 sublist reversals performed in the execution of `@rev_sort([8,6,5,7,9,4])`, where each column going left to right is a step in the algorithm. The $\text{\textcolor{teal}{light colored}}$ nodes are those in the sorted prefix, $\text{\textcolor{teal!25!darkgray}{dark colored}}$ nodes are candidates to join the sorted prefix. The arrows indicate which elements are swapped as sublists are reversed.
